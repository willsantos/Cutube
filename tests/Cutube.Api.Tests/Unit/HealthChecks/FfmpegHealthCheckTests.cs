...
public class FfmpegHealthCheckTests
{
    [Fact]
    public async Task CheckHealthAsync_ShouldReturnHealthCheckResult()
    {
        // Arrange
        var healthCheck = new FfmpegHealthCheck();
        ...
        // In CI or if ffmpeg is not installed, it will be Unhealthy
        // In local dev with ffmpeg installed, it will be Healthy
        if (result.Status == HealthStatus.Healthy)
        {
            result.Description.Should().Contain("ffmpeg is available");
        }
        else
        {
            result.Description.Should().Match(x => x.Contains("ffmpeg is not installed") || x.Contains("ffmpeg check failed"));
        }
    }
